  ...
  for(snr_val in snr){
    ...
    for(rho_within_val in rho_within){
      ...
      for(tau_val in tau){
        ...
        # 5) The contamination proportion
        ...
        output <- lapply(1:length(sparsity), function(t) return(list()))
        ...
        for(sparsity_ind in 1:length(sparsity)){
          ...
          output[[sparsity_ind]] <- Simulation_Run(N = N, m = m, n = n, p = p, 
                                                   cont_scenario = cont_scenario_val, 
                                                   snr = snr_val, 
                                                   rho_within = rho_within_val, rho_between = rho_between, 
                                                   group_size = group_size, 
                                                   tau = tau_val, sparsity = sparsity[sparsity_ind], 
