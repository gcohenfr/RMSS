...
for(n_size in n){
  ...
  for(y_cont in contaminate_y){
    ...
    # Looping over compounds
    for(compound in colnames(y)){
      
      # Print current compound
      cat("compound: ", compound, "\n")
      
      # Selection of chemical compound
      y_compound <- y[, compound]
      ...
      for(iter in 1:N){
        ...
        y_train <- y_compound[train_id[[iter]]]
        ...
        y_test <- y_compound[-train_id[[iter]]]
      ...
      filename <- paste0("results/results_Glass_", compound,"_n=", n_size, "_y_cont=", y_cont, ".RData")
      save.image(filename)
    }
  }
}
